...
//simulate data for control points	
	...
	for(int i = 0; i < nSamples+1; i++){
		if(x_approx[i] > x[1]-0.001 && x_approx[i] < x[1]+0.001) cout << "CP: (" << r[1] << "," << x[1] << ") point #" << i << ": (" << r_approx[i] << "," << x_approx[i] << ")" << endl;
		if(r_approx[i] > r[1]-0.001 && r_approx[i] < r[1]+0.001) cout << "CP: (" << r[1] << "," << x[1] << ") point #" << i << ": (" << r_approx[i] << "," << x_approx[i] << ")" << endl;
	}
